...
- Abilita il plugin SOLO in questo progetto: scrive enabledPlugins + extraKnownMarketplaces nel
  ...
  necessaria a Prometheus) sia in un blocco del .envrc del progetto (direnv — ciò che le attiva nel
  processo claude al lancio) SIA nel .claude/settings.json del progetto. SOLO in questo progetto, mai
  globalmente. Ricorda all'utente di eseguire `direnv allow`. NON chiedere nulla sulla telemetria: è
  preconfigurata. Per disattivarla, telemetry.enabled=false.
